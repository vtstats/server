...
pub async fn execute(ctx: &AppContext) -> anyhow::Result<JobResult> {
    ...
    let bilibili_channels = list_active_channels_by_platform(Platform::Bilibili, &ctx.pool).await?;
    let twitch_channels = list_active_channels_by_platform(Platform::Twitch, &ctx.pool).await?;

    let _ = join!(
        bilibili_subscriber::run(&bilibili_channels, now, ctx).map_err(|err| {
            tracing::error!("Can't collect subscriber for bilibili: {err}");
        }),
        bilibili_view::run(&bilibili_channels, now, ctx).map_err(|err| {
            tracing::error!("Can't collect view for bilibili: {err}");
        }),
        twitch_revenue::run(&twitch_channels, now, ctx).map_err(|err| {
            tracing::error!("Can't collect revenue for twitch: {err}");
        }),
        twitch_subscriber::run(&twitch_channels, now, ctx).map_err(|err| {
            tracing::error!("Can't collect subscriber for twitch: {err}");
        }),
        youtube_revenue::run(&youtube_channels, now, ctx).map_err(|err| {
            tracing::error!("Can't collect revenue for youtube: {err}");
        }),
        youtube_view_subscriber::run(&youtube_channels, now, ctx).map_err(|err| {
