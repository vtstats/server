...
pub async fn search(search: Search, client: &Client) -> Result<Vec<Stream>, Error> {
    ...
    if let Some(start_at) = search.start_at {
        let _ = write!(&mut filter, " AND {} > '{start_at}'", search.sort_by);
    };

    if let Some(end_at) = search.end_at {
        let _ = write!(&mut filter, " AND {} < '{end_at}'", search.sort_by);
    };
    ...
    let result = index
        ...
        .with_filter(&filter)
        .with_sort(&[&format!("{}:{}", search.sort_by, search.sort_direction)])
        .with_limit(24)
        .execute::<Stream>()
        .await?;
